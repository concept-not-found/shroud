...
public class CglibMethodInvocation extends JapexDriverBase {
	private Count count;

	@Override
	public void prepare(final TestCase testCase) {
		count = new Pretender(new CglibProxy(), Arrays.<Object> asList(new StandaloneCount()), new DefaultMethodInvoker(), new DefaultMethodResolver(), new DefaultUnskilledHandler()).pretend(Count.class);
	}
